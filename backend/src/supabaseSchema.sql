...
create table if not exists payments (
  ...
  paid_date date,
  status text not null default 'Outstanding' check (status in ('Paid','Outstanding','Overdue','Partially Paid')),
  method text,
  notes text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
create table if not exists maintenance_requests (
  ...
  status text not null default 'Pending' check (status in ('Pending','In Progress','Scheduled','Completed','Resolved','Closed')),
